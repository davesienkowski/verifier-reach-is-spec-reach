...
\author{David Sienkowski\\\small research and measurements}
...
is removed. A powered replication ($n=121$
verdicts, three model tiers) reproduces the blind spot at a $94\%$ confident-false-pass rate and shows
that resolving the surfaced edge into the spec converts it to a $94\%$ catch; an independent,
...
plus original measurement, not a net-new concept; the results are direction-finding on small samples
plus a reproducible harness, not powered studies.
\end{abstract}
...
\paragraph{A powered replication (n=121).}
We replicate the blind spot at scale on a purpose-built corpus (four self-validated non-inferable tasks
$+$ one inferable control, $\times$ three tiers $\times$ three reps; $n=121$ verdicts; \code{analyze.mjs}
recomputes every figure deterministically from the recorded \code{verdicts.tsv}). Each task is run under
three spec states. (i) \emph{Narrow} (the omitted edge stripped, no abstain option): non-inferable
confident-false-pass $=\mathbf{94\%}$ (opus $100\%$, sonnet $83\%$, haiku $100\%$; $n=32$)---the
$n=12$ blind spot, reproduced and powered. (ii) \emph{Surfaced-resolved} (the omitted edge written into
the spec as an explicit criterion): catch $=\mathbf{94\%}$, residual false-pass $6\%$ (opus/sonnet
$100\%$, haiku $83\%$)---resolving the edge converts blind-spot into catch, mechanizing
verifier-reach $=$ spec-reach. (iii) \emph{Surfaced-unresolved} (the \emph{real} compiled edge-probe tag
appended, abstention available): $33\%$ false-pass, $47\%$ abstain, $19\%$ catch, tier-gated---opus
$100\%$ honest (abstain$+$catch), sonnet $67\%$, haiku $33\%$ (flag-deaf). Two further results matter:
the residual false-pass concentrates exactly on the one task the edge-probe's recall \emph{misses}
(precision/tie: $0\%$ catch even when surfaced---the $\sim\!25\%$ recall gap where a held-out backstop
is irreplaceable), and the inferable control shows $100\%$ catch with $0\%$ over-abstention---the real
tagger, unlike a deliberately-false oracle, did not induce needless deferral.
...
(\S\ref{sec:measurement}, $n=121$) shows the same shape with a \emph{real} edge-probe tag:
surfaced-unresolved drives $47\%$ honest abstention, tier-gated (opus $100\%$ honest, haiku $0\%$), and
surfaced-\emph{resolved} reaches $94\%$ catch; the independent CI-committed eval (Table~\ref{tab:t5})
...
replication is $n=121$ but over only four non-inferable tasks (so the $\sim\!25\%$ recall-gap estimate
rests on $1/4$ categories) and three reps; the independent corroboration is $n\approx10$--$14$ per
...
$\sim\!14\%$ at sonnet, near-zero at opus), but the weakest tier is high-variance and unreliable. On an edge the probe fails to \emph{name}, the verifier
still catches $0\%$---so the pipeline is not a complete fix, and a held-out backstop remains necessary
for the residue.
...
\paragraph{Reproducibility.} The corpora, harnesses, and the merged feature PRs are public. The
$n=121$ replication ships its recorded \code{verdicts.tsv} and \code{analyze.mjs}, which recomputes
every figure deterministically (no model calls). The independent corroboration is a committed, key-free,
