...
ACCEPTANCE (resolved empty/null edge): a key present in `target` with value `null` is PRESENT and must NOT be overridden by fallback — `defaults({ a: null }, { a: 'x' })` MUST return `{ a: null }`.
